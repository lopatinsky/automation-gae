...
from models import MenuCategory, MenuItem, STATUS_UNAVAILABLE, STATUS_AVAILABLE


def do():
    category = MenuCategory(id=2, title=u"Напитки", status=STATUS_AVAILABLE)
    ...
    def _add_item(title, price, beans=(), **kwargs):
        if not beans:
            items.append(MenuItem(id=_next_id(), title=title, price=price, **kwargs))
        else:
            for bean in beans:
    ...
    _add_item(u"Айс-латте", 200, status=STATUS_UNAVAILABLE)
    ...
    _add_item(u"Латте крем-брюле", 350, status=STATUS_UNAVAILABLE)
    _add_item(u"Латте базилик", 350, status=STATUS_UNAVAILABLE)
    _add_item(u"Латте груша-бадьян", 350)
    _add_item(u"Латте шалфей", 350)
    _add_item(u"Клюквенный чай", 350)
    _add_item(u"Чай огурец с ромашкой", 350, status=STATUS_UNAVAILABLE)
