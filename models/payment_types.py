...
class PaymentType(ndb.Model):  # self.key.id() == type
    ...
    def get(cls, payment_id):
        ...
        if app_kind == AUTO_APP:
            ...
            return resto_get_payment_type(str(payment_id))
        elif app_kind == DOUBLEB_APP:
            return doubleb_get_payment_type(str(payment_id))
    ...
    def fetch_types(cls, **kwargs):
        ...
        if app_kind == AUTO_APP:
            ...
            if app_kind == RESTO_APP:
                types = resto_get_payment_types()
            else:
                types = doubleb_get_payment_types()
            for type in types[:]:
