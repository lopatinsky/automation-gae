...
class PromoCondition(ndb.Model):
    ...
    CHOICES = (CHECK_TYPE_DELIVERY, CHECK_FIRST_ORDER, CHECK_MAX_ORDER_SUM, CHECK_ITEM_IN_ORDER, CHECK_REPEATED_ORDERS,
               ...
               CHECK_REPEATED_ORDER_BEFORE, CHECK_MAX_USES, CHECK_MIN_DATE, CHECK_MAX_DATE)
...
OUTCOME_MAP = {
    ...
    PromoOutcome.MARKED_DISCOUNT: u'Скидка на все продукты (метка)',
    PromoOutcome.MARKED_ACCUMULATE_GIFT_POINT: u'Баллы за помеченные продукты (метка)',
    PromoOutcome.MARKED_FIX_DISCOUNT_CHEAPEST: u'Фиксированная скидка на самый дешевый продукт в заказе (метка)',
}
