...
def ga_track_page(tracking_id, hostname, page, title, cid=None, v=1):
    ...
    try:
        urlfetch.fetch('http://www.google-analytics.com/collect', urllib.urlencode(req_values), 'POST', deadline=30)
    except Exception as ex:
        logging.exception(ex)
    return cid_g


def ga_track_event(tracking_id, category, action, label=None, value=None, cid=None, v=1):
    cid_g = cid if cid is not None else gen_ga_cid()
    req_values = {
        'v': v,  # Version.
        'cid': cid_g,  # Anonymous Client ID.
